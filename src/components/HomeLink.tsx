 ...
 *
 * SIZED AGAINST THE SUNGLASSES: 30px (24 on the first pass) (2026-08-23, Noah: "make the home page
 ...
 * public/favicon.svg).
 *
 * REVERSED IN DARK MODE (2026-08-24) — Noah: "I changed my mind on an
 * earlier decision. Let's make sure the 'C' logo... reverses to white when
 * in dark mode." That earlier decision was a hardcoded #231f20 fill that
 * never changed with the theme, on the grounds that this is a fixed brand
 * mark like any other. Swapped for `var(--color-ink)`, the same token every
 * other surface reads — #0d0d0d in light mode (indistinguishable from the
 * old hardcoded value) and #fff in dark, so the mark now inverts along with
 * everything else rather than fading against a dark background. */
export default function HomeLink() {
  ...
  return (
    <Link
      ...
      /* VISIBLE OVER THE FOOTER TOO (2026-08-29). Noah: "let's make sure
         the 'C' logo is still visible when it scrolls into the footer."
         It was painted in --color-ink, which is the right colour against the
         page and the same colour as the footer's own black curtain — so the
         mark vanished into it exactly when the reader reached the bottom.
         `mix-blend-difference` against white is what the theme toggle in the
         opposite corner has always used for the same reason: it inverts
         against whatever is behind it, so the mark is dark on the page and
         light on the footer without either state being chosen. */
