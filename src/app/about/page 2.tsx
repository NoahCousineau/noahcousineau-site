...
function ArcText({
  ...
  const centerY = flip ? cy + radius : cy - radius;
...
function BottomArcText({
  ...
  // radius flattens the arch instead of just relocating it.
  ...
  return (
    ...
    >
      ...
      <section
        ...
        style={{ padding: "calc(var(--u) * 60) calc(var(--u) * 120) calc(var(--u) * 140)" }}
      >
        <a
          ...
          style={{ width: "calc(var(--u) * 1050)", height: "calc(var(--u) * 330)", marginTop: "calc(var(--u) * 150)" }}
        >
          <ArcText
            id="resume-arc"
      ...
      {/* ================= ABOUT-PAGE FOOTER ================= */}
      {/* Custom footer for THIS PAGE ONLY (2026-08-18, round 5) — Noah:
          "the about me page is going to have a custom footer that
          differs from the other pages. It will be just one long
          'cousineau' logo." Simplified from the old contact-info +
          social-links + logo footer down to just the wordmark, spanning
          nearly the full width. This file (src/app/about/page.tsx) is
          the ONLY place this footer lives — the shared global Footer
          component (src/components/Footer.tsx, rendered on every other
          page via layout.tsx) is untouched. */}
      <footer
